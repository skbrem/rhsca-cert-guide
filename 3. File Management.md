...
> [!note]
> - **Absolute pathnames**: Also known as an absolute pathname, this is the complete path reference to a directory or file. It begins at the root directory, and will include all the subdirectories leading up to the current working directory or filename. 

> [!note]
> - **Relative pathnames**: This is relative to the current directory, shown with the `pwd` command. Possible to get from the current directory up to the file that's needed. 
